...
import { parseXML, formatXML } from "@/lib/utils/xml";
...
export function XmlDiff() {
  ...
  const format = () => {
    if (leftResult?.ok)  setLeft(formatXML(left, 2));
    if (rightResult?.ok) setRight(formatXML(right, 2));
  };

  const swap  = () => { setLeft(right); setRight(left); setDiffCount(0); };
  const clear = () => { setLeft(""); setRight(""); setDiffCount(0); };

  const handleUploadLeft  = async () => { const c = await uploadText(".xml"); if (c !== null) setLeft(c); };
  ...
  return (
    <div className={cn("space-y-4", fullscreen && "flex-1 min-h-0 flex flex-col")}>
      ...
      <div className="grid grid-cols-2 shrink-0 gap-14 px-4">
        <PanelLabel actions={<PanelButton icon={<Icons.Upload />} title="Upload XML" onClick={handleUploadLeft} />}>— XML A</PanelLabel>
        <PanelLabel actions={<PanelButton icon={<Icons.Upload />} title="Upload XML" onClick={handleUploadRight} />}>— XML B</PanelLabel>
      </div>
...
type ParseResult = { ok: true; rootTag: string; elementCount: number } | { ok: false; error: string } | null;
