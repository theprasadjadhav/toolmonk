...
import { parseXML, minifyXML } from "@/lib/utils/xml";
...
export function XmlMinifier() {
  ...
  const output = result?.ok ? minifyXML(input) : "";
  ...
  const handleDownload = () => { if (output) downloadText(output, "minified.xml", "application/xml"); };
    ...
    <div className={cn("space-y-4", fullscreen && "flex-1 min-h-0 flex flex-col")}>
      ...
      <div className={cn("grid grid-cols-1 lg:grid-cols-2 gap-4", fullscreen && "flex-1 min-h-0 [grid-template-rows:1fr_1fr] lg:grid-rows-1")}>
        <div className={cn(fullscreen ? "flex flex-col gap-2 min-h-0" : "space-y-2")}>
          ...
          <CodeEditor value={input} onChange={setInput} placeholder='<root><item>minify XML here</item></root>'
        ...
        <div className={cn(fullscreen ? "flex flex-col gap-2 min-h-0" : "space-y-2")}>
          ...
          <CodeOutput value={output} placeholder="minified output appears here"
